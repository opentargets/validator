...
class MinimalTests(unittest.TestCase):
    ...
    def test_enum_fails(self):
        """Incorrect data must raise a validation error."""
        incorrect_filename = os.path.join(self.resources_path, "resources", "minimal.enum.incorrect.json")
        with open_source(incorrect_filename) as data_fh, open_source(self.schema_uri) as schema_fh:
            valid = validate(data_fh, schema_fh)
            self.assertFalse(valid)
    ...
    def test_unexpected_field_fails(self):
        """Incorrect data must raise a validation error."""
        incorrect_filename = os.path.join(self.resources_path, "resources", "minimal.field.incorrect.json")
        with open_source(incorrect_filename) as data_fh, open_source(self.schema_uri) as schema_fh:
            valid = validate(data_fh, schema_fh)
            self.assertFalse(valid)
